...
def run(vid,nsearch=15,ps_f=7):
    ...
    with th.no_grad():
        ...
        for t in range(vid_pad.shape[-3] - 6):
            ...
            min_d_i,min_i_i = find_nn(sliding_window,nsearch)
...
def find_nn(seq_pad,nsearch):
    ...
    i_arange_patch_pad = th.arange(b * f * (h - 14) * (w - 14), dtype=th.long,
    ...
    for t_s in range(7):
        ...
        for v_s in range(nsearch):
            ...
            for h_s in range(nsearch):
                ...
                tmp_i = i_arange_patch_pad + ((t_s - 3) * (h - 14) * (w - 14) +
                                              (v_s - smid) * (w - 14) + h_s - smid)
